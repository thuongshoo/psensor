...
static void apply_config(UI_psensor *ui)
{
    config_sync();
...
static void update_pref(Psensor *s)
{
    ...
    char smin[PSENSOR_MAX_VALUE_LEN];
    if (s->min == UNKNOWN_DOUBLE_VALUE)
        snprintf(smin, sizeof(smin), _("Unknown"));
    else
        psensor_value_to_string_buffer(s->type, s->min, temperature_unit, smin, sizeof(smin));

    gtk_label_set_text(w_sensor_min, smin);

    char smax[PSENSOR_MAX_VALUE_LEN];
    if (s->max == UNKNOWN_DOUBLE_VALUE)
        snprintf(smax, sizeof(smax), _("Unknown"));
    else
        psensor_value_to_string_buffer(s->type, s->max, temperature_unit, smax, sizeof(smax));
...
static void select_sensor(Psensor *s, const Psensor **sensors)
{
    ...
    for (s_cur = sensors; *s_cur; s_cur++, i++)
        if (s == *s_cur)
        {
            GtkTreePath *p = gtk_tree_path_new_from_indices(i, -1);
            GtkTreeSelection *sel = gtk_tree_view_get_selection(w_sensors_list);
...
// Callback để duyệt model và lưu thứ tự
static gboolean save_order_foreach(GtkTreeModel *model, GtkTreePath *path,
                                   GtkTreeIter *iter, gpointer user_data)
{
    ...
    if (depth > 0 && sensor != NULL)
    {
        ...
        // g_print("       Saved: %s (ID: %s) -> Position: %u\n",
        //         name, sensor->id, order);
    }
...
// Hàm lưu tất cả thứ tự khi dialog đóng
static void save_all_sensor_positions(GtkListStore *aStore)
{
    gtk_tree_model_foreach(GTK_TREE_MODEL(aStore),
                           save_order_foreach,
                           NULL);
}

static gboolean is_order_changed = FALSE;

static void on_row_changed(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer user_data)
{
...
{
    if (is_order_changed)
        save_all_sensor_positions(store);
...
static GtkBuilder *load_ui(UI_psensor *ui)
{
    ...
    guint ok = gtk_builder_add_from_file(builder,
    ...
    g_signal_connect(store, "row-inserted", G_CALLBACK(on_row_changed), NULL);
    g_signal_connect(store, "row-deleted", G_CALLBACK(on_row_changed), NULL);
...
void ui_sensorpref_dialog_run(Psensor *sensor, UI_psensor *ui)
{
    if (w_dialog == nullptr)
    {
